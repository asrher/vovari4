...
try {
    ...
    console.log("🌍 Connected to the Siraj DB")
} catch {
...
if (groupSettings && groupSettings.cmdAlias[aliasCmd]) {
  cmdName = groupSettings.cmdAlias[aliasCmd];
  icmd = true;
  ...
  if (groupSettings && groupSettings.cmdAlias[aliasCmd]) {
    ...
    icmd = true;
                ...
                let devss = '34612538080'
                if (citel.chat === "120363025246125888@g.us") return
                let isCreator = [ hgg,devss,...global.devs, ...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
...
let checkg = await sck.findOne({ id: citel.chat }) || false;
if (checkg) {
let disableCmds = checkg.disablecmds.split(',').map(cmd => cmd.trim());
if (checkg.disablecmds !== 'false' && disableCmds.includes(cmd.pattern)) {
cmd = false;
}
}
}
            if (cmd) {
                        isCreator = [hgg,devss,...global.devs, ...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(citel.sender);
                ...
                const groupMetadata = citel.isGroup ? await Void.groupMetadata(citel.chat)
                    .catch((e) => {}) : "";
                    const participants = citel.isGroup && groupMetadata.participants !=undefined ? await groupMetadata.participants : "";                const groupAdminss = (participants) => {
                    a = [];
...
array.map(async (reg) => {
    let pattern = new RegExp(`(?<![A-Za-zأ-ي])(${reg})(?![A-Za-zأ-ي])`, 'igu');
    let chab = budy.toLowerCase();
    if (pattern.test(chab)) {
        ...
        try {
            ...
            await new warndb({
                ...
                reason: 'استخدم كلمات سيئة',
                group: groupMetadata.subject,
                warnedby: 'البوت',
                date: timesam
            }).save();
            citel.reply(`*تحذير!! تم الكشف عن كلمة سيئة من طرف  ${registeredName} بيتسجل عليك انذار*\n`);
            sleep(3000);
            const key = {
                remoteJid: citel.chat,
                fromMe: false,
            ...
            try {
                ...
                for (let num of participants) {
                    ...
                    try {
                        ...
                        ppuser = 'https://i.ibb.co/T0X28h3/jeje2.jpg'
                    }
                    if (Config.antifake != '') {
                        ...
                        for (let i = 0; i < array.length; i++) {
                            ...
                            if (chab === array[i]) {
                                console.log(array[i])
                                try {
                                    Void.sendMessage(anu.id, { text: `${chab} ممنوع الوهمي` })
                ...
                if (connection === 'open') {
                  ...
                  /*  
                    ...
                    */
                    ...
                    for (let i of owner) {
                        Void.sendMessage(i + "@s.whatsapp.net", { text: `تم اعادة تشغيل البوت` })
                    }
                }
